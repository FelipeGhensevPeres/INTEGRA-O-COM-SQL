...
def deletar_insumo():
    ...
    cursor.execute(f'''
                DELETE FROM 
                        Estoque
                WHERE
                    Produto = "{nome_insumo.get()}" AND Lote={lote_insumo.get()}
                    ''')
    cursor.commit()
    conexao.commit()
...
def consumir_insumo():
    ...
    caixa_texto.delete("1.0",END)
    caixa_texto.insert("1.0",f"Insumo {nome_insumo.get()} foi consumido em quantidade:{qtde_insumo.get()} com sucesso!")
